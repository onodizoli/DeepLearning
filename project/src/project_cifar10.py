...
from project_utils import load_cifar10
from project_nn import DropConnectMLP, train_nn, LeNetConvPoolLayer, LocallyConnectedLayer


def test_dropout_model1(learning_rate_zero=0.001, L1_reg=0.00, L2_reg=0.00, n_epochs=[150,0,0],
             batch_size=128, n_hidden=64, n_hiddenLayers=1,
    ...
    # Zero padding
    zeros = numpy.zeros((batch_size, nkerns[0], 32, 32), dtype=theano.config.floatX)
    layer1_input = theano.shared(zeros)
    layer1_input = T.set_subtensor(layer1_input[:, :, 9:-10, 9:-10], layer0.output)
    ...
    # Zero padding
    zeros = numpy.zeros((batch_size, nkerns[1], 32, 32), dtype=theano.config.floatX)
    layer2_input = theano.shared(zeros)
    layer2_input = T.set_subtensor(layer2_input[:, :, 9:-10, 9:-10], layer1.output)
    ...
    if momentum <1:
        ...
        updates = []
        # Update teakes into concsideration current gradient and the previous steo
        for param in params:
            # Storeing previous step
...
def test_dropout_model2(learning_rate_zero=0.001, L1_reg=0.00, L2_reg=0.00, n_epochs=[700,300,50],
             batch_size=128, n_hidden=128, n_hiddenLayers=1,
             verbose=False, activation = 'relu', momentum=1):
    """
    Wrapper function for training and testing MLP
    ...
    # Zero padding
    zeros = numpy.zeros((batch_size, nkerns[2], 32, 32), dtype=theano.config.floatX)
    ...
    if momentum <1:
        updates = []
        # Updating parameters based on current and previous iteration
        for param in params:
            # Saving current step
            param_update = theano.shared(param.get_value()*0., broadcastable=param.broadcastable)
            updates.append((param, param - learning_rate*param_update))
