...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_layer, func):
        if func==1:
            activf = T.nnet.softmax
        else:
            activf = T.tanh
        ...
        # of the hidden layer
        self.logRegressionLayer = LogisticRegression(
            input=self.hiddenLayer[n_layer-1].output,
            n_in=n_hidden,
            n_out=n_out
        )
        # L1 norm ; one regularization option is to enforce L1 norm to
        ...
        for i in range (0,n_layer):
            hiddensum += abs(self.hiddenLayer[i].W).sum()
            hiddensum2 += (self.hiddenLayer[i].W ** 2).sum()
        self.L1 = (
            hiddensum
            + abs(self.logRegressionLayer.W).sum()
        )            
        ...
        # the parameters of the model are the parameters of the two layer it is
        # made out of
        self.params = self.hiddenLayer[0].params
        for i in range(1,n_layer):
            self.params += self.hiddenLayer[i].params
        self.params += self.logRegressionLayer.params
        #self.hiddenLayer[1].params + 
        # keep track of model input
        self.input = input    
 

# TODO: you might need to modify the interface
def test_mlp(activfunc, hidden_layers, n_hidden, learning_rate, L1_reg, L2_reg, n_epochs=500,
        ...
        for minibatch_index in range(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    score_register.append(test_score)
                    if verbose:
    ...
    return score_register, test_score, end_time
