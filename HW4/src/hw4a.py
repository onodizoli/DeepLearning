...
class RNNSLU2(object):
    ...
    """
    def __init__(self, nh, ng, nc, ne, de, cs, normal=True):  #self, nh, ng, nc, ne, de, cs
        ...
        self.wx = theano.shared(name='wx',
                                ...
                                (de * cs, nh))
                                .astype(theano.config.floatX))
        self.wl = theano.shared(name='wl',
                                value=0.2 * numpy.random.uniform(-1.0, 1.0,
                                (nh, ng))
                                .astype(theano.config.floatX))        
        self.wh0 = theano.shared(name='wh0',
                                value=0.2 * numpy.random.uniform(-1.0, 1.0,
                                (nh, nh))
                                .astype(theano.config.floatX))
        self.wh1 = theano.shared(name='wh1',
                                value=0.2 * numpy.random.uniform(-1.0, 1.0,
                                (ng, ng))
                                .astype(theano.config.floatX))        
        self.w = theano.shared(name='w',
                               ...
                               (ng, nc))
                               .astype(theano.config.floatX))
        self.bh0 = theano.shared(name='bh0',
                                value=numpy.zeros(nh,
                                dtype=theano.config.floatX))
        self.bh1 = theano.shared(name='bh1',
        ...
        # bundle
        self.params = [self.emb, self.wx, self.wl, self.wh0, self.wh1, self.w,
        ...
        def recurrence(x_t, h0_tm1, h1_tm1):
            h0_t = T.nnet.sigmoid(T.dot(x_t, self.wx)
                                 + T.dot(h0_tm1, self.wh0) + self.bh0)
            h1_t = T.nnet.sigmoid(T.dot(h0_t, self.wl)
                                 + T.dot(h1_tm1, self.wh1) + self.bh1)            
            
            s_t = T.nnet.softmax(T.dot(h1_t, self.w) + self.b)
            return [h0_t, h1_t, s_t]

        [h0, h1, s], _ = theano.scan(fn=recurrence,
                                sequences=x,
